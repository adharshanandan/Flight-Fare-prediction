...
app=Flask(__name__)
model=pickle.load(open('Flight_price_prediction_RF.pkl','rb'))
...
def predict():
    if request.method=='POST':
        ...
        #Duration
        val1=pd.to_datetime(date_arr,format='%Y-%m-%dT%H:%M')
        val2=pd.to_datetime(date_dep, format="%Y-%m-%dT%H:%M")
        diff=abs(val1-val2)
        seconds=diff.total_seconds()
        def convert(seconds):
            
            hours = seconds // 3600
            bal_seconds=seconds%3600
            minutes=bal_seconds//60
            return hours,minutes
        
        Duration_Hour,Duration_Minute=convert(seconds)
        ...
        #Prediction
        y_preds=model.predict([[Journey_month, Journey_day, Dep_Hour,
